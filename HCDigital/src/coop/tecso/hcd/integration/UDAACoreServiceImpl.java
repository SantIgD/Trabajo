...
public final class UDAACoreServiceImpl implements UDAACoreService {
	...
	private <T extends AbstractEntity> void genericSync(Class<T> clazz, String tableName, Map<String, Object> params, String wsName) throws Exception {
		...
		try {
			...
			int version = 0;
			int serverVersion = tablesToSynchronize.get(tableName.toLowerCase()) != null ? tablesToSynchronize.get(tableName.toLowerCase()) : 0;
	...
	private <T extends AbstractEntity> void synchronize(Class<T> clazz, Map<String, Object> params, String wsName, int version, int serverVersion, TablaVersion tv, boolean partial, String tableName) throws Exception {
		...
		try {
			...
			List <T> itemsChanged = processTableSynchronizeResponse(jsonResponse, clazz);
			...
			if(!ex.getMessage().equals("ERROR_SINCRONIZACION")){
				...
				String message = String.format("Error sincronización de tabla: '%s'; jsonResponse: %s. ERROR: %s", tableName, jsonResponse, ex.getMessage());
				generateACRA_LOG(message, "SINCRONIZACION");
				if(!partial){
					throw new Exception("ERROR_SINCRONIZACION");
				}
	...
	private <T extends AbstractEntity> List<T> processTableSynchronizeResponse(String jsonResponse, Class<T> clazz){
	...
	public void updateTablesToSync() {

		Map<String, Integer> localTablesIndex = getLocalTablesVersionIndex();

		Map<String, Integer> serverTablesVersionIndex = getServerTablesVersionIndex();

		obtainTablesToSyncrhonize(localTablesIndex, serverTablesVersionIndex);

		for (String key : tablesToSynchronize.keySet()) {
			Log.d(LOG_TAG, String.format("%s --> %d", key, tablesToSynchronize.get(key)));
		}

	}


	private void obtainTablesToSyncrhonize(Map<String, Integer> localTablesIndex, Map<String, Integer> serverTablesVersionIndex ){

		tablesToSynchronize = new HashMap<>();

		for (String tableName : localTablesIndex.keySet()) {
			Integer serverVersion = serverTablesVersionIndex.get(tableName);
			Integer localVersion = localTablesIndex.get(tableName);
			...
			if (serverVersion > localVersion || forceUpdate) {
				tablesToSynchronize.put(tableName, serverVersion);
			}
		}

	}

	private Map<String, Integer> getServerTablesVersionIndex() throws RuntimeException{

		String response = requestServerTableVersion();

		Map<String, Integer> serverTablesVersionIndex = new HashMap<>();
		...
		for (JsonElement jsonElement: json.getAsJsonArray()) {
			...
			serverTablesVersionIndex.put(key, value);
		}

		//No debo tener en cuenta esta tabla
		serverTablesVersionIndex.remove("hcd_atencion");

		return serverTablesVersionIndex;
	}

	private String requestServerTableVersion() throws RuntimeException{
		Map<String, Object> params = new HashMap<>();
	...
	private Map<String, Integer> getLocalTablesVersionIndex(){
		...
		//No debo tener en cuenta esta tabla
		localTables.remove("hcd_atencion");
		return localTables;
	}
	...
	private boolean isSynchronized(String tableName){
		...
		if(!tablesToSynchronize.isEmpty() && tablesToSynchronize.get(tableName.toLowerCase()) == null){
