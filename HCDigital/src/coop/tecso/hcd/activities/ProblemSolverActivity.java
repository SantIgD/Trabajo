...
public class ProblemSolverActivity extends Activity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        Button deleteGenericData = findViewById(R.id.delete_generic_data_buttons);
        deleteGenericData.setOnClickListener(view -> confirmAndDeleteGenericData());

        cantClases = DatabaseConfigUtil.PERSISTENT_CLASSES.length - 1;
    ...
    private void wipeAllData() throws SQLException {
        Class<?>[] tablesToDelete = new Class<?>[cantClases];
        int i = 0;
        for (Class<?> clazz: DatabaseConfigUtil.PERSISTENT_CLASSES) {
            if (!clazz.equals(TablaVersion.class)) {
                tablesToDelete[i] = clazz;
                i++;
            }
        }
        deleteDataOfClass(tablesToDelete);
    ...
    private void deleteAtenciones(){ // No anda bien, cuando elimina luego no reconoce que tiene un IM proveniente del servidor
        try{
            for (Class<?> clazz: DatabaseConfigUtil.PERSISTENT_CLASSES) {
                if(clazz.equals(Atencion.class) || clazz.equals(AtencionValor.class) || clazz.equals(AtencionCerrada.class) ) {
    ...
    private void confirmAndDeleteGenericData(){
        int title = R.string.confirm_title;
        int mensaje = R.string.delete_generic_data_msg;

        AlertDialog alertDialog = GUIHelper.createAlertDialog(this,title,mensaje, this::deleteGenericData,null);
        alertDialog.show();
    }

    private void deleteGenericData(){
        Class<?>[] classesToDelete = new Class<?>[cantClases-3];
        int i = 0;
        for (Class<?> clazz: DatabaseConfigUtil.PERSISTENT_CLASSES) {
            if( clazz.equals(Atencion.class) || clazz.equals(AtencionCerrada.class)
                                             || clazz.equals(AtencionValor.class)
                                             || clazz.equals(TablaVersion.class)){
                continue;
            }
            classesToDelete[i] = clazz;
            i++;
        }
        TablaVersionDAO tablaVersionDAO = new TablaVersionDAO(this);
        try {
    ...
    private void deleteDataOfClass(Class<?>[] clases){
        for (Class<?> clazz: DatabaseConfigUtil.PERSISTENT_CLASSES) {
            if(CollectionUtils.contains(clases,clazz)){
                DatabaseHelper databaseHelper = OpenHelperManager.getHelper(this, DatabaseHelper.class);
                RuntimeExceptionDao<AbstractEntity, Integer> entidadBusquedaDAO = databaseHelper.getRuntimeExceptionDao((Class<AbstractEntity>) clazz);
                DeleteBuilder<AbstractEntity, Integer> deleteBuilder = entidadBusquedaDAO.deleteBuilder();
